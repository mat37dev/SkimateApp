...
export function useUserLocation(options: GeoOptions = {}) {
	...
	const [location, setLocation] = useState<[number, number] | null>(fallback);
		...
		async function requestPermission() {
			if (Platform.OS === "android") {
				const granted = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION, {
					...
					message: "This app needs access to your location to show your position in real time.",
					buttonPositive: "OK",
				});
...
export async function getLocation(): Promise<[number, number] | null> {
	...
	if (Platform.OS === "android") {
		const granted = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION, {
			title: "Location Permission",
			message: "Skimate needs access to your location to center the map.",
			buttonPositive: "OK",
		});
